...
| 발견 | 증상 | 제안 |
|------|------|------|
| ① newNum 슬롯 위치 변위 | 문단 0.14(머리말+하이퍼링크 필드 복합 문단)에서 newNum(PAGE 새번호) ctrl이 fieldEnd 뒤로 이동 — char_offsets[3] 27→35 (8유닛). placeholder 없는 8-슬롯 타입의 위치 결함 (autoNum과 별개 계열) | 별도 이슈 등록 |
| ② 143E RT 페이지 수 1→2 | 모든 시대 RT 동일, tbl pageBreak(#1393) 패치로도 불변 — 원인 미규명 (머리말 컨트롤 내용 등 후보) | ①과 묶어 등록 (같은 샘플 추적) |

(tbl pageBreak RowBreak→CellBreak 자체는 #1393 기지)

## 5. 한컴 판정 요청

1. **`output/poc/task1382/ta-pic-001-r.rt.hwpx`** — 캡션 번호가 **"&lt;그림 1&gt;"
   원위치**에 표시되는지 (#1387 재판정의 "문장 끝 밀림" 해소 확인). 셀 안 그림
   크기는 #1389 기지 — 범위 밖.
2. (선택) **`samples/hwpx/exam_social.hwpx` 원본**을 한컴에디터에서 열어 2페이지
   짝수 머리말의 "쪽번호+고정폭 공백" 모양과 rhwp 렌더(3.4절 앵커 갱신분)의 시각
   비교 — fwSpace 스타일 귀속 정정의 정답 확인용.
