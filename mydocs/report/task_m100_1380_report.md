...
- **form-002** (4단계, 여백 #1388 통제 후): 전 10쪽 텍스트 요소 수 동일
  ...
  - 짝수 페이지 절 제목 27자 y +5.6~7.4px 시프트 — **표 pageBreak 속성 미보존**
    (4.1 신규 발견) 기인 개연. `ir-diff` 잔존 차이가 tbl page_break 5건뿐임을 확인
...
## 4. 신규 발견 (범위 밖 — 별도 이슈 등록 제안)

| # | 발견 | 증상 | 발견 시점 |
|---|------|------|----------|
| 1 | MEMO 필드(fieldBegin) subList 소실 | aift `type="MEMO"` 2건의 parameters+메모 본문 문단 RT 소실 — 게이트 사각 | 1단계 |
| 2 | shapeComment 소실 | aift `hp:shapeComment` 15→0 | 1단계 |
| 3 | 표 pageBreak 속성 미보존 | 원본 `pageBreak="CELL"/"NONE"` → RT 일괄 `"TABLE"` 방출. IR page_break 변형 (form-002 5건, RowBreak→CellBreak) — 표 분할 배치 시프트 기여. 게이트 사각(컨트롤 내부 속성 비교 없음) | 4단계 |

작업지시자 승인 시 각각 이슈 등록한다.
...
| MEMO subList·shapeComment·tbl pageBreak | 신규 — 4절 등록 제안 |
