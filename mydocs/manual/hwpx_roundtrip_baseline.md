# HWPX Roundtrip Baseline 가이드 (Task #1315, #1378 게이트 강화)
...
> **중요**: baseline 통과 = 시각 충실도 보장이 **아니다**. 문단 내부 컨트롤 수/내용,
> lineseg 값은 비교하지 않으므로 셀 내 그림 소실·페이지 수 변화도 baseline을 통과할
> 수 있다 (#1315 4단계 보고서 실증). 시각 판정 권위는 작업지시자(한컴에디터)에게 있다.
...
추가로 **임시 xfail** `XFAIL_1378_RECURSIVE` 13건 — #1378 게이트 재귀 확장으로
검출된 범위 밖 실패 (subList 컨트롤 미출력 #1379 기인 12건 + 파서 autoNum 폭
비일관 1건). 후속 이슈 해소 시 `xfail_1378_recursive_entries_still_fail` 가드가
실패하므로 목록·skip 블록을 제거하고 baseline으로 승격한다.
...
| `xfail_1378_recursive_entries_still_fail` | 임시 xfail(#1378 재귀 확장분)의 승격 누락 방지 가드 |
...
| 셀·글상자 subList 컨트롤 미출력 | 셀 내 그림/컨트롤 소실 + char_shapes 경계 8 유닛 배수 시프트 | #1379 |
| lineseg 원본 보존 불완전 | 페이지 수 변화 (form-002 10→17쪽, 보도자료 9→13쪽), 1~2px 시프트 | #1380 |
| `exam_social.hwpx` borderFillIDRef 31 미등록 | SERIALIZE_FAIL — xfail 등록됨 | #1381 |
| 파서 autoNum 폭 비일관 | char_shapes 축 1 유닛 vs offsets 축 8 유닛 — 재파싱 경계 1 유닛 시프트 (143E433F503322BD33 각주) | 별도 이슈 후보 |
