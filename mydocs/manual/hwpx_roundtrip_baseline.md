...
| ~~표 캡션(`hp:caption`) 미직렬화~~ | ~~캡션 subList 문단 소실 (전수 17건 중 표 6건)~~ | **#1387 해소** — write_caption + TableCaption 게이트 동승 (RT 가능 표 캡션 5/5 복원). 그림/도형 캡션 11건은 별도 이슈 |
| 캡션 내 autoNum 슬롯 변위 | 파서가 placeholder를 1유닛 적재 → slot 추론 mismatch → 끝 방출 + 재파싱 끝 공백 1자 (ta-pic-001-r) — 텍스트 축이라 게이트 비가시 | #1382 발현 (캡션 경로 신규 결함 아님) |
